...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    ...
    class Woo_Custom_Thank_You_Page {
        ...
        public function add_plugin_menu() {
            add_submenu_page(
                'woocommerce',
                __('Woo Custom Thank You', 'woo-custom-thank-you-page'),
                __('Thank You Page', 'woo-custom-thank-you-page'),
                'manage_woocommerce',
        ...
        public function settings_page() {
            ?>
            <div class="wrap">
                <h1><?php _e('Woo Custom Thank You Page', 'woo-custom-thank-you-page'); ?></h1>
                <p class="description" style="max-width: 700px; font-size: 14px; color: #555;">
                    <?php _e('Set a <strong>Global Thank You Page</strong> that applies to all products unless a specific Thank You Page is selected at the product level. This allows you to have either a unified experience for all customers or a tailored message per product.', 'woo-custom-thank-you-page'); ?>
                </p>
                        ...
                        <tr valign="top">
                            <th scope="row">
                                <label for="woo_global_thankyou_redirect_page">
                                    <?php _e('Global Thank You Page', 'woo-custom-thank-you-page'); ?>
                                    <span class="dashicons dashicons-editor-help" id="help-tip-global-thankyou" title="<?php _e('This page will be used after checkout unless a product-specific thank you page is defined.', 'woo-custom-thank-you-page'); ?>"></span>
                                </label>
                            </th>
                            <td>
                                <select name="woo_global_thankyou_redirect_page">
                                    <option value=""><?php _e('-- Select Page --', 'woo-custom-thank-you-page'); ?></option>
                                    <?php
        ...
        public function add_product_meta_box() {
            add_meta_box(
                ...
                __('Thank You Page Redirect', 'woo-custom-thank-you-page'),
        ...
        public function render_product_meta_box($post) {
            ...
            echo '<label for="woo_product_thankyou_page">' . __('Select a Thank You Page:', 'woo-custom-thank-you-page') . '</label><br />';
            echo '<select name="woo_product_thankyou_page" id="woo_product_thankyou_page">
                    <option value="">' . __('-- Default (Global) --', 'woo-custom-thank-you-page') . '</option>';
            foreach (get_pages() as $page) {
                printf(
                    '<option value="%s" %s>%s</option>',
                    $page->ID,
        ...
        public function redirect_thankyou_page() {
            ...
            if (!$order) {
                wp_die(__('Order not found.', 'woo-custom-thank-you-page'));
                return;
            }
    ...
    add_filter('plugin_action_links_' . plugin_basename(__FILE__), function($links) {
        $settings_link = '<a href="' . esc_url(admin_url('admin.php?page=woo-custom-thank-you')) . '">' . __('Settings', 'woo-custom-thank-you-page') . '</a>';
    ...
    add_action('admin_notices', function() {
        ...
        _e('Woo Custom Thank You Page requires WooCommerce to be installed and active.', 'woo-custom-thank-you-page');
        echo '</p></div>';
    });
}
